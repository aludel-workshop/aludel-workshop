...
export const initial = () => ({version:3, state:'Blocked', revisions:[{id:1,text:request}], decision:0, scope:null, deferred:false, auth:[], attempts:[], artifacts:[], reviews:[], events:[], connector:'Not mapped', mapping:null, scenario:'Happy path', selected:null});
...
export function act(s,type,value) {
 ...
 case 'decision': if(['Running','Queued','Authorized','Cancelling','Cancellation uncertain','Awaiting decision'].includes(s.state)) throw Error('Resolve the active attempt first'); s.scope=value; s.decision++; s.deferred=false; if(rev) s.state='Ready'; break;
 ...
 case 'answer': requireState('Awaiting decision'); if(!value?.trim()) throw Error('Answer required'); s.inputAnswer=value;s.state='Continue ready';s.scenario='Happy path';break;
 ...
 case 'accept': {requireState('Awaiting review');const a=s.artifacts.find(a=>a.id===s.selected);if(!a || stale(s,a))throw Error('This artifact is stale');s.reviews.push({artifact:a.id,kind:'Accepted',digest:a.digest,revision:a.revision,decision:a.decision});s.state='Completed';break}
 case 'revision': {requireState('Awaiting review');if(!value?.trim())throw Error('Feedback required');s.reviews.push({artifact:s.selected,kind:'Revision requested',feedback:value});newRevision(value);s.state='Ready';break}

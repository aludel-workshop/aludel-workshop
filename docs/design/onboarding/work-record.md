...
| ONB-06 Project workspace scoping | ONB-05 | New projects open in the same Overview/Product/Work workspace (the destination of **Continue in Aludel**), pages become a page/view record kind in Product, and the remaining hard-coded `the-machine` references are removed | Two-project isolation checks from the PW-01 plan | — |

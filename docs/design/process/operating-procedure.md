...
Apply the **purpose test** to every new tab, record kind or container: name its producer (a role action), its consumer (an action or view that reads it) and its anchor (the record it belongs to), or show that it is a derived view. A container nobody reads, or one attached to nothing, is not ready. Also ask whether it duplicates a structure that already exists; ROADMAP-01's opportunity tree mirrored the story map. In a prototype, label each new composition as borrowed (name the reference screen) or invented, and describe deterministic actions by their rule, saving "propose" and "suggest" for agent output. ROADMAP-01 found that two of six Product tabs failed this test after they had shipped. Re-run it on a layer's existing tabs at the start of each UX pass and whenever a shared structure lands: DESIGN-UX-01 found Sources and Guidelines duplicated the Library, which was built after them. First application: [ROADMAP-01 §4](../product-and-plan/work-record.md#4-the-purpose-test-applied-to-todays-product-tabs); unproven until its review.

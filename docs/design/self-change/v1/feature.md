---
id: design-self-change-feature-v1
...
Help the owner understand which unanswered choices prevent the current milestone, resolve one in context, and keep independent work moving. The first delivered change is a blocker summary on Project Overview with links to decision and work records. Its development also exercises request → interpretation → bounded authorization → identified preview → review.

Those are two related layers: **the feature being delivered** is milestone-decision visibility; **the surrounding workflow** is how the owner commissions and reviews that feature. The review board labels its snapshots so a preview of Overview is not confused with the live portal surrounding it.
...
```text
Projects → Aludel → Overview [V1: orient, request, find consequence]
...
| Fixture | Meaning |
|---|---|
| Current milestone | M1 — Request, run, and review |
| Q-002, unanswered | What may a task authorization allow? Blocks implementation of task execution, T-EXEC |
| PORTAL-001/r1 | Request to add milestone-decision visibility; its trial execution awaits the example authorization decision |
| T-COPY, ready | Clarify decision descriptions; depends on neither Q-002 nor the execution implementation |
| Q-VIS, optional | Wording preference for milestone titles; no blocking link |
| A1 | Synthetic preview of the completed self-change using Q-002/r1 and PORTAL-001/r1 |
...
These are open research questions. The author's preferred direction is A; no usability finding or owner selection is implied.

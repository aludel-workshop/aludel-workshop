---
id: design-borrowbox-contracts-v2
...
| Open required question | Consequence and affected work; review decision | BB-D01 counted once despite two task links |
| Optional preference | Separate optional section | BB-D03 not counted as blocking |
...
| Answer recorded | Confirmation + inspect ready task | No automatic execution; BB-002 unchanged |
...
## BorrowBox application boundary

C1–C10 are portal components. The artifact shown in C9 belongs to BorrowBox and contains a tool catalog, tool details, date fields, availability feedback, and a reservation receipt. Their behavior and deterministic examples are defined in the [demo fixture](../../demo-app/borrowbox.md). They do not contain portal decisions or worker state. The current board draws only the tool/date preview; the app is not implemented.
